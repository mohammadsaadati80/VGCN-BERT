...
if will_train_mode_from_checkpoint and os.path.exists(os.path.join(output_dir, model_file_save)):
    ...
    perform_metrics_prev = 0
    best_macro_f1 = 0
    model = Vanilla_VGCN_Bert.from_pretrained('./bert', gcn_adj_dim=gcn_vocab_size, gcn_adj_num=len(gcn_adj_list),gcn_embedding_dim=gcn_embedding_dim, num_labels=len(label2idx))
    prev_save_step=-1
